...
def about(subject):
    ...
    def inner(paragraph):
        ...
        return any(word in split(paragraph) for word in subject)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if cur <= limit and cur < diff:
            res = word
            diff = cur
    return res and res or typed_word
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[time[j + 1] - time[j] for j in range(len(time) - 1)] for time in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_idx in word_indices:
        time = min(times[player_idx][word_idx] for player_idx in player_indices)
        for player_idx in player_indices:
            if times[player_idx][word_idx] == time:
                res[player_idx].append(words[word_idx])
                break
    return res
    # END PROBLEM 10
