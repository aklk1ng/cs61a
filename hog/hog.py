...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    assert 0 <= player_score, "scores should be a non-negative."
    assert 0 <= opponent_score, "scores should be a non-negative."
    rhs = (opponent_score // 10) % 10
    lhs = player_score % 10
    res = 3 * abs(rhs - lhs)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(goal):
        for score1 in range(goal):
            if strategy(score0, score1) != initial:
                return False
    return True
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    Now just change the threshold and num_rolls paramters.
    """
    # BEGIN PROBLEM 12
    return 0 if boar_brawl(score, opponent_score) >= 9 else 6
    # END PROBLEM 12
