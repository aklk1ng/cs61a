...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur = self.place
        step = 0
        # Bee in the hive cannot be attack
        while cur is not None and cur.entrance is not None:
            if self.valid_place(step) and cur.bees:
                return random_bee(cur.bees)
            step += 1
            cur = cur.entrance
        return None

    def valid_place(self, distance):
        return distance <= self.upper_bound and distance >= self.lower_bound
...
class HungryAnt(Ant):
    """HungryAnt select a random Bee from its place and deal damage to the Bee equal to the Bee's health, eating it whole.
    After eating a Bee, it must spend 3 turns chewing before being able to eat again.
    """
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        """The ContainerAnt does not already contain another ant and the other ant is not a container."""
...
class TankAnt(ContainerAnt):
    """TankAnt is a ContainerAnt but also deals 1 damage to all bees in its place each turn."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is more costly and waterproof."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        cur = self.place.exit
        while cur is not None and cur.exit is not None:
            if cur.ant:
                cur.ant.double()
